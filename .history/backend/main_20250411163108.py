    ...
    async def event_generator():
        ...
        try:
            ...
            for _ in range(rounds):
                ...
                accumulated = ""
                stream = await arbitrator_agent.make_judgment(proposal, challenge)
                async for chunk in stream:
                    if hasattr(chunk, 'content'):
                        yield DebateSSEMessage(
                            role=arbitrator,
...
arbitrator_runnable = RunnableLambda(lambda x: Arbitrator().make_judgment(x["proposal"], x["challenge"]))

...
const startDebate = () => {
  ...
  const messageHandler = (event) => {
    ...
    // 原有消息处理逻辑
    // 确保不重复处理相同消息
    const existingMsgIndex = debateResult.value.messages.findIndex(msg => 
      msg.content === data.content && msg.role === data.role && msg.type === data.type
    )
  ...
  // 确保只绑定一次事件监听器
  currentEventSource.addEventListener('message', messageHandler)
}

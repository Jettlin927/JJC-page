...
const startDebate = () => {
  ...
  currentEventSource.addEventListener('round_end', () => {
    showContinue.value = true
    showReset.value = true  // 每轮结束后也显示重新开始按钮
    isLoading.value = false
    console.log('Round end - showContinue:', showContinue.value) // 调试日志
  })
...
<template>
  <div class="max-w-7xl mx-auto p-8 text-center min-h-screen">
    ...
    <div v-if="showReset || showContinue" class="fixed bottom-8 left-0 right-0 flex justify-center gap-4">
      <button
        v-if="showContinue"

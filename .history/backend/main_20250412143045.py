...
from backend.agents.proposer import Proposer
from backend.agents.challenger import Challenger
from backend.agents.arbitrator import Arbitrator
        ...
        try:
            ...
            async for chunk in stream:
                if hasattr(chunk, 'content'):
                    ...
                    yield DebateSSEMessage(
                        role=proposer,
                        content=chunk.content,  # 发送当前chunk内容
                        type="proposal"
                    ).model_dump_json()
